 ...
 * dígitos, guardado de la mascota en Firestore, y sistema de amigos
 * (solicitud/aceptar/rechazar/eliminar). Todo lo que habla con Firebase
 * vive en este único archivo — el resto de la app (js/app.js) sólo llama
 * a las funciones de `window.Cloud`, sin importar si hay nube o no.
 *
 ...
 * El login es sólo nombre de usuario + PIN de 4 dígitos, sin cuenta de
 * verdad detrás (sin mail, sin recuperación). Esto es a propósito — es
 * un juego para jugar entre amigos de confianza, no una app con datos
 * sensibles. El PIN se guarda hasheado (SHA-256), nunca en texto plano,
 * y las reglas de Firestore (firestore.rules) exigen que cualquier
 ...
 * usuario, y evita que un script use la app a ciegas sin el PIN. NO es
 * autenticación real: como no hay backend propio, una persona con
 * conocimientos técnicos que inspeccione las peticiones de red podría
 * leer el hash guardado (la lectura está abierta, hace falta para poder
 * buscar amigos por nombre) y reenviarlo sin conocer el PIN real. Para
 * el uso previsto (mascota virtual entre compañeros/amigos) es un techo
 * de protección razonable; no lo uses para nada que necesite seguridad
 * de verdad.
 */
